...
%% LSTM error model v1 %%
LSTM_error_v1 = importTensorFlowNetwork('../LSTM/LSTM_error_NN_model_v1');
%% load data %%
...
y_test_LSTM_error_v1 = predict(LSTM_error_v1, x_train) + x_train;

error_PNN = zeros(length(data), 10);
error_LSTM = zeros(length(data), 10);
error_LSTM_error = zeros(length(data), 10);
error_LSTM_error_v1 = zeros(length(data), 10);
for i = 1:length(data)
    ...
    [delta_r_eb_n,delta_v_eb_n,delta_eul_nb_n] = Calculate_errors_NED(...
    y_test_LSTM_error_v1(i, 1),y_test_LSTM_error_v1(i, 2),y_test_LSTM_error_v1(i, 3),y_test_LSTM_error_v1(i, 4:6)',...
    Euler_to_CTM(y_test_LSTM_error_v1(7:9)'),true_L_b,...
    true_lambda_b,true_h_b,true_v_eb_n,true_C_b_n);

    error_LSTM_error_v1(i,1) = in_profile(i, 1);
    error_LSTM_error_v1(i,2:4) = delta_r_eb_n';
    error_LSTM_error_v1(i,5:7) = delta_v_eb_n';
    error_LSTM_error_v1(i,8:10) = delta_eul_nb_n';
end
