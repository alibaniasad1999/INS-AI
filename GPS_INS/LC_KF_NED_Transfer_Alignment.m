function [est_v_eb_n_new,est_C_b_n_new,est_IMU_bias_new,P_matrix_new] =...
    LC_KF_NED_Transfer_Alignment(GNSS_v_eb_n,tor_s,L,lambda,h,est_v_eb_n,est_C_b_n_old,est_IMU_bias_old,...
...
Q_prime_matrix = [Q11        Q21'        zeros(3)    Q15;
       Q21        Q22         Q24         Q25;
       zeros(3)   Q24'        Q44         zeros(3);
       Q15'       Q25'        zeros(3)    Q55;];
%==========================================================================
%% Propagation 
...
H_matrix(1:3,1:3)   =  -Skew_symmetric(est_C_b_n_old * cross(meas_wBIB,lGBB) - WEIN * est_C_b_n_old * lGBB);
H_matrix(1:3,4:6)   = -eye(3);
H_matrix(1:3,10:12)   = -est_C_b_n_old * Skew_symmetric(lGBB);


p = [L; lambda; h];
%Formulate measurement innovations using 1.53:
% delta_z(1:3,1) = Sp * (GNSS_r_eb_n - p - T_prn * est_C_b_n_old * lGBB);
delta_z(1:3,1) = GNSS_v_eb_n - est_v_eb_n - est_C_b_n_old * cross(meas_wBIB,lGBB)+ WEIN * est_C_b_n_old * lGBB;
%--------------------------------------------------------------------------
...
R_matrix(1:3,1:3) = eye(3) * LC_KF_config.vel_meas_SD^2;
%==========================================================================
%% Estimation
...
P_matrix_new = (eye(12) - K_matrix * H_matrix) * P_matrix_propagated;
%==========================================================================
...
est_IMU_bias_new = est_IMU_bias_old + x_est_new(7:12);
%==========================================================================
end